...
* `host`: Хост на котором располагается хранилище данных, например: '127.0.0.1',
* `namespace`: Пространство имен для хранения ключей, например: 'my-site',
* `get`: Объект с полем 'port' для конфигурации хранилища для обработки запросов на чтение данных, например: { port: 3000 },
* `post`:  Объект с полем 'port' для конфигурации хранилища для обработки запросов на изменение данных, например: { port: 3001 },
* `auth`: Строка в которой должен располагаться заголовок для авторизации польщователя,
