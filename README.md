...
Конфигурация инструмента описывается в файле `config/config.json`.
...
* `repositoriesFileName` - имя .json файла c описанием репозиториев данные из которых попадут в результат сборки.
* `dataRepository` - конфигурация удаленного репозитория для выкладки собранных данных и хранения
...
содержимым локального файла `repositoriesFileName`, а не скачиваться с удаленного репозитория.

Файл `repositoriesFileName` скачивается с удаленного репозиотрия, в который также отправляются собранные данные после сборки.
Структуру этого файла можно разделить на 2 группы `private` и `public`. В разделе `private` описываются репозитории, находящиеся
